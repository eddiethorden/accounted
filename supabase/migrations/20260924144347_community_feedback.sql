-- Community feedback on shared agent instructions (Agentinstruktioner).
--
-- 1. What kind of item a shared instruction is (flow, knowledge, analysis),
--    chosen by its author at submission. Stored on the submission itself
--    (company_skills), frozen with the rest of the submitted content, and
--    read through published_atom_id once the reviewer has published it.
-- 2. community_feedback: one row per (community item, user). A vote and a
--    "fungerar / fungerar inte" answer, independent of each other. A user
--    reads and writes only their own row; everyone sees the counts through
--    community_item_stats(), which returns aggregates and public listing
--    data only, never a company or user.

-- 1. The kind of a shared item
ALTER TABLE public.company_skills
  ADD COLUMN kind text NOT NULL DEFAULT 'workflow' CHECK (kind IN ('workflow', 'rules', 'analysis'));

-- The author chooses it when submitting; afterwards it is part of the frozen
-- submission, like the name and body (guard_company_skill_change).
...
CREATE TABLE public.community_feedback (
  ...
  feedback text CHECK (feedback IN ('works', 'not_works')),
...
-- Only live, published community items can be rated, and a row stays on its
-- item and its person.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Only live community items can be rated' USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END;
$$;
...
-- whether the author is an accounting firm (a byrå team), vote and answer
-- counts, and how many companies loaded it (mcp.skill_loaded, as far back as
...
RETURNS TABLE (
  ...
  author_shared integer, author_verified boolean, votes integer, works integer, not_works integer, used_by integer
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $$
  ...
  answers AS (
    SELECT f.atom_id,
      (count(*) FILTER (WHERE f.vote))::integer AS votes,
      (count(*) FILTER (WHERE f.feedback = 'works'))::integer AS works,
      (count(*) FILTER (WHERE f.feedback = 'not_works'))::integer AS not_works
  ...
  SELECT i.id, COALESCE(l.kind, 'workflow'), l.author_handle,
    ...
    COALESCE(an.votes, 0), COALESCE(an.works, 0), COALESCE(an.not_works, 0), COALESCE(lo.companies, 0)
  FROM items i
  LEFT JOIN listing l ON l.atom_id = i.id
  LEFT JOIN authors au ON au.author_handle = l.author_handle
  LEFT JOIN answers an ON an.atom_id = i.id
