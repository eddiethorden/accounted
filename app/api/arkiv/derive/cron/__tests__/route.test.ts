...
vi.mock('@/lib/arkiv/facts/derive-company', () => ({ deriveCompanyFacts: vi.fn(async () => ({ recorded: 3, predicates: ['revenue_12m'] })) }))
...
describe('GET /api/arkiv/derive/cron', () => {
  ...
  it('observes every company in the rollout and re-queues agreements older than a week', async () => {
    ...
    expect(body).toEqual({ ok: true, companies: 1, checked: 3, matched: 1, missed: 1, rederived: 1, facts: { companies: 1, recorded: 3, failed: 0 } })
