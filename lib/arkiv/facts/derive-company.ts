...
export interface CompanyFactInputs {
  ...
  /** Counterparties by money moved in the period, from the company graph. */
  counterparties: Array<{ name: string; flow: number }>
...
export function companyFactsFrom(inputs: CompanyFactInputs): FactDraft[] {
  ...
  // Loans: what stands on the balance sheet, from every posting.
  const loanAccounts = inputs.lines.filter((l) => inRange(l.account_number, '2350', '2399') || inRange(l.account_number, '2840', '2849'))
  const loanBalance = loanAccounts.reduce((n, l) => n + l.credit - l.debit, 0)
  if (Math.abs(loanBalance) >= 1) {
    const accounts = [...new Set(loanAccounts.map((l) => l.account_number))].sort()
    add({ predicate: 'loan_balance', value: roundOre(loanBalance), valueText: `${kr(loanBalance)} (${accounts.join(', ')})`, sourceKind: 'ledger', singleValued: true, evidence: { accounts, as_of: inputs.today } })
  }
  ...
  for (const c of [...inputs.counterparties].filter((c) => c.flow > 0).sort((a, b) => b.flow - a.flow).slice(0, TOP_COUNTERPARTIES)) {
    add({ predicate: 'top_counterparty', value: { name: c.name, flow: roundOre(c.flow) }, valueText: `${c.name}: ${kr(c.flow)} (12 mån)`, sourceKind: 'ledger', singleValued: false, evidence: { ...period } })
  }
...
export async function deriveCompanyFacts(supabase: SupabaseClient, companyId: string, today: string, accountName: (account: string) => string | null): Promise<DeriveCompanyFactsOutcome> {
  ...
  const [recent, balance, employees, settings, company, banks, held] = await Promise.all([
    ...
    base().or('and(account_number.gte.2350,account_number.lte.2399),and(account_number.gte.2840,account_number.lte.2849)').limit(5000),
  ...
  const counterparties = graph.nodes.filter((n) => n.kind === 'party' || n.kind === 'merchant').map((n) => ({ name: n.label, flow: Number((n.meta as { flow?: number }).flow ?? 0) }))
